...
export function generateUcsSteps(
  ...
  const openSet = new MinHeap<GridNode>((a, b) => a.gCost - b.gCost);
  ...
  steps.push({
    ...
    description: "Initialized Uniform Cost Search. Exploring based on actual path cost g(n).",
    highlightedLine: 1,
    state: {
  ...
  while (!openSet.isEmpty()) {
    ...
    steps.push({
      stepIndex: stepCounter++,
      description: `Evaluating node (${current.row}, ${current.col}) with g(n)=${current.gCost}. Checked ${consideredNeighbors.length} neighbors.`,
      highlightedLine: 2,
      state: {
        grid: cloneGrid(workingGrid),
        startPos: { ...startPos },
